...
// src/app/api/creation/projects/[projectId]/storyboard/breakdown-chain —
// see that file for the full design note. Same logic, keyed by
// videoId/scriptId instead of projectId (matches every other storyboard
// route pair in this app).
  ...
  try {
    ...
    for (const { node: n, stage } of assignments) {
      ...
      if (!clip && stageSec >= MIN_STAGE_SEC) {
        ...
        clip = { source: "upload", url: `/api/media/storyboard/${params.scriptId}/${filename}`, kind: "video" };
      }
